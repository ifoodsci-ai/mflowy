...
class TestStopOnErrorMiddleware:
    ...
    def _make_task(self, name, step_type=StepType.LOAD, stop_on_error=True):
        conf = StepConf(
            name=name,
            type=step_type,
            module="test",
    ...
    def test_stop_on_error_true_propagates_exception(self):
        ...
        @handler(StepType.LOAD, stop_on_error)
        ...
        with pytest.raises(ValueError, match="intentional error"):
            _REGISTRY[(StepType.LOAD, "failing_handler")](task)
    ...
    def test_stop_on_error_false_continues(self):
        ...
        # 不应抛出异常
        result = _REGISTRY[(StepType.LOAD, "failing_handler")](task)
    ...
    def test_stop_on_error_true_passes_result(self):
        ...
        result = _REGISTRY[(StepType.LOAD, "ok_handler")](task)
        assert result == "ok"
    ...
    def test_middleware_order_with_checkpoint(self):
        ...
        @handler(StepType.LOAD, mock_checkpoint, stop_on_error)
        def failing_handler(**kwargs):
