...
- 绘图能力实现：`packages/builtin_plugins/mflowy/builtin_plugins/plots/`（目录结构按 data_analysis / model_evaluation / model_interpretability 分类镜像本篇）
- 模型与评估实现：`packages/builtin_plugins/mflowy/builtin_plugins/model/`、交叉验证：`packages/builtin_plugins/mflowy/builtin_plugins/cross_validation/`
