...
def _reset_mlflow_global_state(monkeypatch):
    ...
    # fluent run 栈隔离：mock 了 mlflow.start_run 的用例（如 test_profile）会残留空 ActiveRun，
    # 跨包顺序变化时泄到后续真实 start_run 用例（prior bug：旧 tests/ 字母序恰好掩盖）
    # ThreadLocalVariable 无 token reset API，用前后双 set 清空代替
    fluent._active_run_stack.set([])
